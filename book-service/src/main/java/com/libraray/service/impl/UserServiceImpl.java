...
import com.libraray.config.JwtTokenUtil;
import com.libraray.dto.*;
import com.libraray.entity.User;
import com.libraray.enums.Role;
import com.libraray.exception.AccessDeniedException;
import com.libraray.exception.InvalidLoginException;
import com.libraray.exception.UnauthorizedException;
import com.libraray.exception.UserNotFoundException;
import com.libraray.repository.PersonRepository;
import com.libraray.repository.UserRepository;
import com.libraray.service.AuthenticationService;
import com.libraray.service.UserService;
import com.libraray.service.UserValidator;
import com.libraray.utils.SpecificationUtils;
...
public class UserServiceImpl extends PersonServiceImpl<User> implements UserService {
    ...
    public AuthenticationResponse authenticateUser(SignIn loginDto) {
        try {
            ...
            return new AuthenticationResponse(user.getId(), user.getEmail(), user.getFirstName(), user.getLastName(), user.getUsername(),
                     user.getPhone(),user.getGender(), user.getPassword(), user.getRole(), user.getAddress(),
                    accessToken);
    ...
    public List<PersonDTO> searchUsers(UserSearchRequest searchRequest) {
        ...
        Specification<User> spec = (root, query, cb) -> {
            List<Predicate> predicates = SpecificationUtils.createPersonPredicates(
                    root, cb, searchRequest.getEmail(), searchRequest.getFirstName(),
                    searchRequest.getLastName(), searchRequest.getPhone(), searchRequest.getGender(), searchRequest.getAddress()
            );


//                    searchRequest.getContactDetails(),
    ...
    public PersonDTO findUserByIdOrEmailOrUsername(Long id, String email, String username) {
        ...
        if (loggedInUser.getRole() == Role.ROLE_CLIENT ||
                (loggedInUser.getRole() == Role.ROLE_LIBRARY && (user.getRole() == Role.ROLE_LIBRARY || user.getRole() == Role.ROLE_ADMIN))) {
